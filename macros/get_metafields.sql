{%- macro duckdb__get_metafields(source_object, reference_values, id_column, lookup_object="stg_shopify__metafield", key_field="metafield_reference", key_value="value", reference_field="owner_resource") -%}
...
{# DuckDB-compatible: guard against NoneType returned by max_columns for unrecognized adapters #}
{%- set _max_columns = shopify.max_columns(source_column_count, id_column) -%}
{%- set pivot_fields = dbt_utils.get_column_values(
    table=ref(lookup_object),
    column=key_field,
    max_records=[var('shopify_max_metafields', 50), _max_columns]|sort|first if (_max_columns is not none and target.type != 'snowflake') else var('shopify_max_metafields', 50),
