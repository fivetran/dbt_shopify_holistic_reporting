...
{% set shopify_model = ref('shopify__orders') if var('shopify_api', 'rest') == 'rest' else ref('shopify_gql__orders') %}
...
with orders as (
    ...
    select 
        {{ dbt_utils.star(from=shopify_model, except=['source_relation']) }},
